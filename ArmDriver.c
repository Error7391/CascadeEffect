...
float iShoulderRatio(int sSetting) {
  int raw = sSetting - sctrl.shoulderZero;
  float angle = raw * -45 / 15;
...
task ElbowControl () {
  while(true) {
    ...
    wait1Msec(100);
    if(sctrl.engaged && sctrl.activeElbow) {
...
task ShoulderControl () {
  while(true) {
    ...
    //writeDebugStreamLine(" >>> ShoulderControl - engaged is %d, target is %d, currentPos is %d",sctrl.engaged,sctrl.targetPos,sctrl.currentPos);
    if (sctrl.engaged && sctrl.currentPos != sctrl.targetPos) {
      int diff = sctrl.targetPos - sctrl.currentPos;
      int delta = (diff < 0) ? -1 : 1;
      sctrl.currentPos += delta;
      //writeDebugStreamLine(" >>> ShoulderControl - delta is %d, diff is %d, currentPos is %d",delta,diff,sctrl.currentPos);
      servo[shoulder] = sctrl.currentPos;
      if (iShoulderRatio(sctrl.targetPos) > 100 && iShoulderRatio(sctrl.currentPos) > 90) {
        sctrl.activeElbow = true;
      } else if (iShoulderRatio(sctrl.targetPos) < 90) {
        sctrl.activeElbow = true;
      } else {
        sctrl.activeElbow = false;
      }
    }
  }
}
...
void armInit(TArmState& tasr) {
  ...
  sctrl.engaged = false;
  sctrl.shoulderZero = tasr.shoulderZero;
  sctrl.activeElbow = false;
...
void setArmAngle(TArmState& tasr, int angle) {
  if (angle >= 0 && angle <= 180) {
    int sangle = tasr.shoulderZero + shoulderRatio(angle);
    int eangle = tasr.elbowZero + elbowRatio(angle);
    writeDebugStreamLine("SET ARM ANGLE to %d - shoulder = %d, elbow = %d",angle,sangle,eangle);

    //servo[shoulder] = (sangle < tasr.shoulderZero) ? tasr.shoulderZero:sangle;
    //servo[elbow] = eangle;
    sctrl.targetPos = (sangle > tasr.shoulderZero) ? tasr.shoulderZero:sangle;
  } else {
